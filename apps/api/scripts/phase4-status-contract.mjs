...
assert(['operational', 'degraded'].includes(status.status), 'Overall status is invalid.');
...
for (const forbidden of ['ipAddress', 'lastError', 'storageKey', 'userId', 'exception']) {
  assert(!serialized.includes(forbidden), `Public status leaked internal field ${forbidden}.`);
}
...
assert(proxiedStatus.status === status.status, 'Web and API status contracts disagree.');

const [page, nav] = await Promise.all([
  readFile('../web/src/app/status/page.tsx', 'utf8'),
  readFile('../web/src/components/public-nav.tsx', 'utf8'),
]);
assert(page.includes('60_000') && page.includes('رخدادهای جاری'), 'Status page refresh or incident UI is incomplete.');
assert(nav.includes('href="/status"'), 'Public navigation does not link to Status Page.');

console.log('Phase 4 status contract passed: public component health, sanitized incidents, BFF access, refresh UI, and navigation.');
