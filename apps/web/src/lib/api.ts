...
function requestKey(path: string, method: string, body: unknown) {
  if (body instanceof FormData) return `${method}:${path}:form:${crypto.randomUUID()}`;
...
async function execute<T>(path: string, options: RequestOptions): Promise<T> {
  const {
    method = 'GET', body, auth = true, isFormData = false, signal,
...
export function apiFetch<T = unknown>(path: string, options: RequestOptions = {}): Promise<T> {
