    ...
    <main id="main-content" className="mx-auto w-full max-w-4xl px-4 py-12 md:px-8">
      ...
      {data && <>
        <Card className={`mt-6 ${data.status === 'operational' ? 'border-success-border bg-success-subtle' : 'border-warning-border bg-warning-subtle'}`}>
          <div className="flex items-center justify-between gap-3"><div><p className="text-lg font-extrabold text-fg">{data.status === 'operational' ? 'همه سرویس‌ها در دسترس‌اند' : 'برخی سرویس‌ها با کاهش کیفیت مواجه‌اند'}</p><p className="mt-1 text-sm text-fg-muted">آخرین بررسی: {new Date(data.generatedAt).toLocaleString('fa-IR')}</p></div><Badge color={data.status === 'operational' ? 'green' : 'yellow'}>{data.status === 'operational' ? 'عملیاتی' : 'کاهش کیفیت'}</Badge></div>
        </Card>
        <section className="mt-8"><h2 className="mb-3 text-lg font-bold text-fg">اجزای سرویس</h2><div className="grid gap-3 md:grid-cols-2">{data.components.map((component) => <Card key={component.id}><div className="flex items-center justify-between gap-3"><div><p className="font-bold text-fg">{component.label}</p>{component.lastActivityAt && <p className="mt-1 text-xs text-fg-subtle">آخرین فعالیت: {new Date(component.lastActivityAt).toLocaleString('fa-IR')}</p>}</div><Badge color={component.status === 'operational' ? 'green' : component.status === 'degraded' ? 'yellow' : 'gray'}>{LABELS[component.status]}</Badge></div></Card>)}</div></section>
        <section className="mt-8"><h2 className="mb-3 text-lg font-bold text-fg">رخدادهای جاری</h2>{data.incidents.length === 0 ? <Card><p className="font-bold text-success">رخداد فعالی گزارش نشده است.</p><p className="mt-1 text-sm text-fg-muted">سامانه طبق آخرین بررسی در وضعیت پایدار قرار دارد.</p></Card> : <div className="space-y-3">{data.incidents.map((incident) => <Card key={incident.id}><div className="flex items-center justify-between gap-3"><div><p className="font-bold text-fg">{incident.title}</p><p className="mt-1 text-xs text-fg-subtle">شروع بررسی: {new Date(incident.startedAt).toLocaleString('fa-IR')}</p></div><Badge color="yellow">{incident.status === 'investigating' ? 'در حال بررسی' : 'پایش'}</Badge></div></Card>)}</div>}</section>
