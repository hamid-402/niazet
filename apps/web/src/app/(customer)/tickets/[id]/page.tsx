    ...
    <div className="mx-auto max-w-2xl">
      ...
      <Card>
        ...
        <div className="flex gap-2">
          <input className={inputClass} value={body} onChange={(e) => setBody(e.target.value)} placeholder="پیام خود را بنویسید..." />
          <Button
            disabled={busy || !body}
            onClick={async () => {
              setBusy(true);
              try {
                await apiFetch(`/customer/tickets/${id}/messages`, { method: 'POST', body: { body } });
                setBody('');
                load();
              } catch (err) {
                setError(err instanceof ApiError ? err.message : 'خطا در ارسال پیام');
              } finally {
                setBusy(false);
              }
            }}
